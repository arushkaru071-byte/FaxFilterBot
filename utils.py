...
async def send_all(bot, userid, files, ident, chat_id, user_name, query):
    ...
    try:
        ...
        await query.answer('UnBlock the Main Bot first !', show_alert=True)
    except PeerIdInvalid:
        await query.answer('Hey, Start the Bot first and Click on Send All', show_alert=True)
    except Exception as e:
        await query.answer('Hey, Start the Bot first and Click on Send All', show_alert=True)
    ...
    if settings["imdb"]:
        ...
        if IMDB_CAP:
            ...
            if imdb:
                ...
                cap = f"<b><i>The Results for ☞ {search}\n\nRequested By ☞ {query.from_user.mention}\n\nResults shown in ☞ {remaining_seconds} seconds\nPowered by ☞ : {query.message.chat.title}\n\n<blockquote>⚠️ After 5 minutes this message will be Automatically Deleted 🗑️</blockquote>\n\n</i></b>"
                cap+="<b><i>🍿 Your Movie Files 👇</u></b>\n\n"
                for file in files:
                    cap += f"<b>📁 <a href='https://telegram.me/{temp.U_NAME}?start=files_{file['file_id']}'>[{get_size(file['file_size'])}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file['file_name'].split()))}\n\n</a></b>"
    else:
        cap = f"<b><i>The Results for ☞ {search}\n\nRequested by ☞ {query.from_user.mention}\n\nResults shown in ☞ {remaining_seconds} seconds\n\nPowered by ☞ : {query.message.chat.title} \n\n⚠️ After 5 minutes this mesaage will be Automatically Deleted 🗑️\n\n</i></b>"
        cap+="<b><i>🍿 Your Movie Files 👇</i></b>\n\n"
        for file in files:
            cap += f"<b>📁 <a href='https://telegram.me/{temp.U_NAME}?start=files_{file['file_id']}'>[{get_size(file['file_size'])}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file['file_name'].split()))}\n\n</a></b>"
    return cap
